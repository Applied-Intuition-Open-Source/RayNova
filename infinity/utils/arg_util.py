...
class Args(Tap):
    ...
    resume_path: str = '' # 's3://research-datasets/users/chensheng_peng/ckpts/infinity/Infinity-fsbx3y_04bd200000_0/ar-gpt_model_latest.pth' # 's3://research-datasets/users/chensheng_peng/ckpts/infinity/Infinity-kqqonq_042cf00000_0/ar-gpt_model_latest.pth' #'s3://research-datasets/users/chensheng_peng/ckpts/infinity/Infinity-49kscb_0459700000_0/gpt_model_latest.pth' # 's3://research-datasets/users/chensheng_peng/ckpts/infinity/Infinity-scale-spatial-temporal-box/ar-ckpt-last_0019.pth' # 's3://research-datasets/users/chensheng_peng/ckpts/infinity/multi_view_mutli_time_v6t4_motionray/ar-ckpt-last_0059.pth'               # pretrained infinity checkpoint
    ...
    fsdp_orig: bool = True
    enable_checkpointing: str = "full-block"    # checkpointing strategy: full-block, self-attn
    pad_to_multiplier: int = 1          # >1 for padding the seq len to a multiplier of this
    log_every_iter: bool = True
    ...
    saln: bool = True                  # whether to use a shared adaln layer
    ...
    fp16: int = 2                       # 1: fp16, 2: bf16, >2: fp16's max scaling multiplier todo: 记得让quantize相关的feature都强制fp32！另外residueal最好也是fp32（根据flash-attention）nn.Conv2d有一个参数是use_float16？
    ...
    log_freq: int = 10                  # log frequency in the stdout
    gclip: float = 6.                   # <=0 for not grad clip VAE
    dclip: float = 6.                   # <=0 for not grad clip discriminator
    tclip: float = 5.                   # <=0 for not grad clip GPT; >100 for per-param clip (%= 100 automatically)
    ...
    ada: str = '0.9_0.97'                       # adam's beta0 and beta1 for VAE or GPT, '0_0.99' from style-swin and magvit, '0.5_0.9' from VQGAN
    ...
    enable_dynamic_length_prompt: int = 1 # enable dynamic length prompt during training
    ...
    object_cond_drop_rate: float = 0.25 # Probability to drop box conditions
    ...
    class_names: list = None            # [automatically set; don't specify this]
    map_names: list = None              # [automatically set; don't specify this]
