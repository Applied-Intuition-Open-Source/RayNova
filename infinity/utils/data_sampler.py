...
class InfiniteGroupEachSampleInBatchSampler(Sampler):
    ...
    def __init__(self, 
        ...
        self.size = (len(dataset) + self.global_batch_size - 1) // self.global_batch_size
        assert self.groups_num >= self.global_batch_size, f"Number of groups must be >= global batch size {self.groups_num}, {self.global_batch_size}"
...
class FiniteGroupEachSampleInBatchSampler(Sampler):
    """
    Sampler for validation dataset, ensuring each sample in a batch comes from a unique group,
    without shuffling, and sampling finitely.
    """

    def __init__(self, dataset, batch_size=1, world_size=None, rank=None, seed=0):
        """
        Args:
            dataset (Dataset): The dataset to sample from.
            batch_size (int): Batch size per GPU.
            world_size (int, optional): Total number of GPUs. Defaults to None.
            rank (int, optional): Rank of the current GPU. Defaults to None.
        """
        
        if world_size is None or rank is None:
            # Assume single GPU if not provided.
            world_size = 1
            rank = 0
        ...
        # Group-to-sample index mapping
        self.group_idx_to_sample_idxs = {
            group_idx: np.where(self.flag == group_idx)[0].tolist()
            for group_idx in range(self.groups_num)}

        # Group indices per global sample index, in sequential order
        self.group_indices_per_global_sample_idx = [
            self._sequential_group_indices(self.rank * self.batch_size + local_sample_idx)
            for local_sample_idx in range(self.batch_size)]

        # Sample index buffers per local sample
        self.buffer_per_local_sample = [[] for _ in range(self.batch_size)]
        
        # Track which group each global sample has used.
        self.group_used = [0] * self.global_batch_size

    def _sequential_group_indices(self, global_sample_idx):
        """Generates sequential group indices for a specific global sample."""
        for i in range(self.groups_num):
            yield i

    def __iter__(self):
        """Yields batches of sample indices."""
        batch_count = 0
        while batch_count < self.size:
            curr_batch = []
            for local_sample_idx in range(self.batch_size):
                global_sample_idx = self.rank * self.batch_size + local_sample_idx

                if len(self.buffer_per_local_sample[local_sample_idx]) == 0:
                    # Refill buffer with next group
                    try:
                        new_group_idx = next(self.group_indices_per_global_sample_idx[local_sample_idx])
                    except StopIteration:
                        # All groups have been used, end sampler
                        return
                    self.buffer_per_local_sample[local_sample_idx] = \
