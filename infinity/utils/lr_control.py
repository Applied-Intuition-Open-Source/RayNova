...
def freeze_params(model, part_to_freeze: str = None):
    """
        Freeze the parameters of the model except for the action head. 
        This is used to finetune the action head with other parts frozen.
    """
    if part_to_freeze is None:
        return model
    
    assert part_to_freeze == 'action', 'only action head is supported for now'

    for name, param in model.named_parameters():
        if 'action' in name or 'act_head' in name or 'command_embedding' in name:
            continue
        else:
