...
class CKPTSaver(object):
    ...
    def get_state_dict_to_save(self, args: arg_util.Args, g_it: int, next_ep: int, next_it: int, trainer):
    ...
    def sav(
        ...
        #NOTE: must start with ar-
        str_next_ep = str(next_ep).zfill(4)
        fname = f'ar-ckpt-last_{str_next_ep}.pth'
...
def auto_resume(args: arg_util.Args, pattern='ckpt*.pth') -> Tuple[List[str], int, int, str, List[Tuple[float, float]], dict, dict]:
    ...
    if args.auto_resume:
        ...
        all_ckpt = [resume]
        info.append(f'[auto_resume] load from @ {resume} ...')
    else:
