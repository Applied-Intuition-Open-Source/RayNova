...
from infinity.raynova_models.raynova import RAYNOVA
...
def concat_text_cond_tuples(text_cond_tuples_list):
    kv_compact_list = []
    lens = []
    max_seqlen_list = []
    device = text_cond_tuples_list[0][0].device
    for text_cond_tuple in text_cond_tuples_list:
        kv_compact_list.append(text_cond_tuple[0])
        lens.extend(text_cond_tuple[1])
    kv_compact = torch.cat(kv_compact_list, dim=0)
    cu_seqlens_k = np.cumsum([0] + lens)
    cu_seqlens_k = torch.tensor(cu_seqlens_k, dtype=torch.int32, device=device)
    max_seqlen = max(lens)
    return (kv_compact, lens, cu_seqlens_k, max_seqlen)
...
def encode_prompt(text_tokenizer, text_encoder, prompt, enable_positive_prompt=False, print_info=True):
...
def gen_one_frame_raynova(
    ...
    for prompt in prompt_list:
        text_cond_tuple = encode_prompt(text_tokenizer, text_encoder, prompt, enable_positive_prompt, print_info=print_info)
        if negative_prompt:
            negative_label_B_or_BLT = encode_prompt(text_tokenizer, text_encoder, negative_prompt, print_info=print_info)
        else:
            negative_label_B_or_BLT = None
        assert negative_label_B_or_BLT is None
        text_cond_tuples_list.append(text_cond_tuple)
    text_cond_tuple = concat_text_cond_tuples(text_cond_tuples_list)
    if print_info:
    ...
    with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16, cache_enabled=True):
        ...
        _, _, img_list = infinity_test.autoregressive_infer_cfg(
            ...
            gt_leak=gt_leak, gt_ls_Bl=gt_ls_Bl, 
            temporal_gt_leak=temporal_gt_leak, temporal_gt_ls_Bl=temporal_gt_ls_Bl, inference_mode=True,
            sampling_per_bits=sampling_per_bits, camera_meta_datas_list=camera_meta_data_list, 
            bbox_sequence=bbox_sequence, map_sequence=map_sequence,
        )
    if print_info:
...
def load_raynova(
    ...
    print(f'[Loading RAYNOVA]')
    text_maxlen = 512
    with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16, cache_enabled=True), torch.no_grad():
        infinity_test: RAYNOVA = RAYNOVA(
            ...
            add_view_embeding_only_first_block=add_view_embeding_only_first_block,
            add_time_embeding_only_first_block=add_time_embeding_only_first_block,
            ...
            bbox_img_coord=bbox_img_coord,
            use_temporal_attn=use_temporal_attn,
            use_condition_rope=use_condition_rope,
        ...
        print(f'[you selected InfinityMVT with {model_kwargs=}] model size: {sum(p.numel() for p in infinity_test.parameters())/1e9:.2f}B, bf16={bf16}')
        ...
        if checkpoint_type == 'torch':
            ...
            if 'bbox_encoder._class_tokens' in state_dict:
...
def load_transformer_raynova(vae, args):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    infinity = load_raynova(
        ...
        view_embed_type=args.view_embed_type,
        attn_bias_type=args.attn_bias_type,
        add_view_embeding_only_first_block=args.add_view_embeding_only_first_block,
        add_time_embeding_only_first_block=args.add_time_embeding_only_first_block,
        object_condition=args.object_condition,
        class_names=args.class_names,
        time_embed_type=args.time_embed_type,
        head_depth=args.head_depth,
        bbox_img_coord=args.bbox_img_coord,
        use_temporal_attn=args.use_temporal_attn,
        use_condition_rope=args.condition_rope,
        map_condition=args.map_condition,
